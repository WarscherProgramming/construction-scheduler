...
function LoadingResults({ data }) {
  ...
  return (
    <>
      ...
      {data.conflicts.length > 0 && <section className="resource-conflicts" aria-labelledby="resource-conflicts-title"><h2 id="resource-conflicts-title">Capacity Conflicts</h2><ul>{data.conflicts.map((conflict) => <li key={`${conflict.resource.resource_type}:${conflict.resource.id}:${conflict.date}`}><strong>{conflict.resource.name} / {formatDisplayDate(conflict.date)}</strong><span>{conflict.message}</span><small>{conflict.contributing_tasks.map((task) => `${task.wbs || task.id} ${task.name}`).join(", ")}</small></li>)}</ul></section>}
